...
modelToPath={'DDA verify':'DDA_verify_path', 'DDA input':'DDA_input_path', 'EvoOpt 2D input':'EvoOpt_2D_input_path'}
        ...
        if filename.endswith(".txt"):
            ...
            if(nameit >= int(itStart) and nameit <= int(itEnd)):
                if shapeSolidPlot=="True":
                    plot_func.Shape(geometry, diel, d, colormax=colorMax,shapebarrier=shapeBarrier,plotDpi=plotDpi, iteration=nameit, position=pos+"ShapeSolid\\", decimal=dec, FullLattice=True)
                if shapePlot=="True":
                    plot_func.Shape(geometry, diel, d, colormax=colorMax,shapebarrier=shapeBarrier,plotDpi=plotDpi, iteration=nameit, position=pos+"Shape\\", decimal=dec, FullLattice=False)
                if xPlot=="True":
