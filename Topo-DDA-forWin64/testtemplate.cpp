...
//2022-2-6 periodic Abs design with 3layer and filter and binarization
int main() {

    string save_position = ".\\IntegrateE-rect88\\";       //output file
    Vector3d l;
    Vector3d center;
    l << 19.0, 19.0, 29.0;    //Size of the initialization block. 81*81*17 pixels in total.
    center << l(0) / 2, l(1) / 2, l(2) / 2;      //Center of the block.
    ...
    Structure s2(S.get_total_space(), l2, center2, &s2_fix);
    Structure s3(S.get_total_space(), l3, center3);
    S = S + s1;                                                         //Add the geometry into the space.
    S = S + s1_rect_start;
    S = S + s2_fix;
    ...
    list<VectorXi*> BPGeometryl{ s2_fixgeometry, s2geometry, s3geometry };
    list<double> BParal{ 1.0, 1.0, 2.0 };
    bool Filter = true;
    double r_f = 2.0;
    vector<filterinfo> filterlist{ filterinfo{0,1.0}, filterinfo{80,2.0} };
    FilterOption filteropt(0.0, 50.0, 0.5, "piecewise", filterlist);
    SpacePara spacepara(&S, bind, FParaInit, FPGeometryl, BPGeometryl, BParal, Filter, &filteropt);
    //SpacePara is where the parameter<->geometry link is established.
